...
## 當前狀況
- 前後端品牌名稱請統一顯示為「立翔水電行」，並於登入頁與應用程式頂部呈現一致的 Logo 與字樣。
...
- 任務支援多重指派，API 會回傳 `assignee_ids` 與 `assignees` 陣列，前端使用多選元件調整對象。
- 使用者可在個人資料頁設定任務通知方式（Email 或 LINE Notify），指派與狀態更新會觸發通知。
- Flask 目前會在處理請求前檢查 JWT：未驗證的非 API 流量會被重新導向至登入頁；所有非 `/api/` 的路由已改為回傳 React build 的 `index.html`，避免重新整理後出現 404。
